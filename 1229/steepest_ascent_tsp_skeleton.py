...
def mutants(current, p): # Apply inversion
    ...
    #     i = random.randint(0, numCities) # 숫자 1개 나오니까, 2개 생성
    #     j = random.randint(0, numCities)
    #     if i > j:
    #         i, j = j, i
    #     mutant.append(inversion(current, i, j))
    #     triedPairs = []
    #     triedPairs.append([i,j])

    #     if [i, j] in triedPairs:
    #         i = random.randint(0, numCities) 
    #         j = random.randint(0, numCities) 
...
def inversion(current, i, j):  ## Perform inversion
    ...
    # current: [1, 2, 3, 4, 5]
    # curCopy = current[:]
    # curCopy1 = current[i:j+1]
    # curCopy1.reverse()
    # curCopy[i:j+1] = curCopy
    # curCopy = current[:]
    # while i < j:
